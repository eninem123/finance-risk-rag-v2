...
## [2.3.0] - 2026-06-25
### ✨ 新增功能
- **PII 合规脱敏**: 引入 `PIIMasker`，自动掩码银行卡、身份证、邮箱及手机号，确保 LLM 交互符合金融合规标准。
- **AI 执行摘要**: `RiskAnalysisService` 现在能自动生成专业、简洁的银行级风险摘要。
- **动态风险矩阵**: `dashboard.py` 集成 Plotly，通过可视化矩阵展现风险实体的影响权重与提取置信度。
- **增强型报告**: 重构 Markdown 报告结构，包含执行摘要、量化评分表及更专业的风控建议。

### 🔧 架构优化
- **延迟加载 (Lazy Init)**: 服务层组件采用 `@property` 模式，大幅减少非必要资源占用。
- **冲突仲裁升级**: 细化了 BERT (0.85+) 与规则提取的仲裁策略，显著提升复合提取的准确率。
- **数据模型增强**: `Entity` 模型新增 `impact_score` 字段，支持更精细的量化风控。
