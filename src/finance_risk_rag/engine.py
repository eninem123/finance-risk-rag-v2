...
class RAGEngine:
    ...
    def _initialize_db(self):
        try:
            ...
            self._emb_fn = ef.ONNXMiniLM_L6_V2(
                ...
                if not force:
                    ...
                    if existing and existing["metadatas"]:
                        ...
                        if existing["metadatas"][0].get("hash") == current_hash:
                            logger.info(
                                f"Skipping indexing for {txt_file.name} (unchanged)"
                            )
                            continue
                        else:
                            # 如果已改变，先删除旧的索引
                            logger.info(
                                f"Updating index for {txt_file.name} (content changed)"
                            )
                            self._collection.delete(where={"source": txt_file.name})

                content = txt_file.read_text(encoding="utf-8")
                cleaned = clean_text(content)
                sentences = split_text_by_sentence(
                    cleaned, max_len=self.config.chunk_size
                )
