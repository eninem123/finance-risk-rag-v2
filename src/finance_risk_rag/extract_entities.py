...
class EntityExtractionPipeline:
    ...
    def __init__(self, config=None):
        ...
        self.logger = setup_logger(
            "extraction_pipeline", str(self.config.log_dir / "extraction.log")
        )
    ...
    def interactive_qa(self, entities: List[Entity]):
        ...
        print("\n--- 交互式风险问答 (输入 'exit' 退出) ---")
        context = "\n".join(
            [f"- {e.text} ({e.type}, 风险分: {e.risk_score})" for e in entities[:10]]
        )
        ...
        while True:
            ...
            try:
                response = self.client.chat.completions.create(
                    model=self.config.llm_model_name, messages=[{"role": "user", "content": prompt}]
                )
